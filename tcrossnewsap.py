...
・p≧0.05の場合は、「有意に高かった／低かった」を使用してはならない。
・p≧0.05の場合は、「高かったが差はなかった」又は「両群で差はなかった」を使用する。
・イベント率の大小のみで有意差を判断してはならない。
・HR解析では、p値を優先して文章化する。
・群名とイベント率の対応を確認してから文章化する。

【棒グラフ・割合比較ルール】

・棒グラフでは、色と群名の対応を確認してから文章化する。
・割合比較では、左右の群を逆転してはならない。
・グラフ内のカテゴリ名を勝手に翻訳、補完してはならない。
・p値はグラフ全体比較に対応する内容として記載する。
・割合のみで「有意に高かった」を記載してはならず、p値を確認する。
・棒グラフでは、凡例の色と群名を確認してから数値を対応させる。
・棒グラフでは、左右位置のみで群を判断してはならない。
・同一カテゴリ内の色対応を優先して文章化する。
・入力画像に存在しない発表者名、結論コメントを生成してはならない。
・棒グラフ比較では、複数カテゴリを1つのvs表現へ圧縮してはならない。
・カテゴリごとに独立した文章で記載する。
・DOACと経口抗凝固薬を別カテゴリとして解釈してはならない。
・複数グラフが存在する場合、各グラフを独立して文章化する。
・別グラフの数値を混在させてはならない。
・棒グラフ上部のn数と割合を、カテゴリ割合として解釈してはならない。

【回帰解析ルール】

・回帰解析では、有意差のある因子を優先して文章化する。
・有意差のない因子を個別列挙してはならない。
・OR、HR解析では、「独立予測因子であった」を使用する。
・「有力な因子」「関連を示した」などの解釈は禁止する。
・単変量解析、多変量解析は独立した文章で記載する。
